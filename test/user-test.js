...
describe('User', function() {
  ...
  it('should have a list of past and present bookings', function() {
    expect(guest1.bookings.length).to.deep.equal(4);
  });

  it('should be able to find the rooms available on a given day', function() {
    let availableRooms = guest2.searchAvailability('2020/02/20', rooms, bookings);
    ...
    let roomsAvailableToday = guest3.searchAvailability(today, rooms, bookings);
    expect(roomsAvailableToday.length).to.deep.equal(7);
  });

  it('should be able to filter available rooms by type', function() {
    let filteredByType = guest1.filterByRoomType(today, rooms, bookings, 'single room');
    expect(filteredByType).to.deep.equal([
      {"number":12,"roomType":"single room","bidet":false,"bedSize":"twin","numBeds":2,"costPerNight":172.09},
  ...
  it('should be able to book a room', function() {
    let availableRooms = guest3.searchAvailability(today, rooms, bookings);
    let selectedRoom = availableRooms[2];
    let newBooking = guest3.bookRoom(today, availableRooms, bookings, selectedRoom);
